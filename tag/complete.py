...
class CompleteException(Exception):
    destfile:str
    reason:str
    def __init__(self,destfile:str,reason:str) -> None:
        Exception.__init__(self)
        self.destfile=destfile
        self.reason=reason

    def __str__(self) -> str:
        return f"为文件 '{self.destfile}' 添加元信息失败:{self.reason}"
...
def complete_from_cloudmusic(destfile: IO[bytes],
    ...
    if search_tag:
        try:
            ...
            raise CompleteException(destfile.name,f"获取 '{destfile.name}' 的标签信息时出错："
        ...
        if not cover_data and cover_url:
            try:
                ...
                raise CompleteException(destfile.name,f"获取 '{destfile.name}' 的封面信息时出错："
                     f"{type(exc).__name__}: {exc}"
                     )
                return_status = False
